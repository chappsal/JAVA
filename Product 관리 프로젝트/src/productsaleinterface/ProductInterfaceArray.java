...
interface ProductInterface {
	int TV=1, COMPUTER=2, AUDIO=3; //상수 앞 public static fianl 생략됨
...
class Buyer {
	...
	//구매자 제품 목록
	//배열 : 반드시 같은 타입만 저장, 크기 고정되므로 최대 크기로 선언, 추가 삭제시 코드 직접 작성해야 하는 번거로움
	...
	void summery() {
		...
		for(i=0; i<item.length; i++) { // i :배열의 index 역할 = 구매자가 구매한 제품 수
			...
			//출력 방법-1 삼항연산자
			itemlist += (i==0) ? item[i].toString() :  ", " + item[i].toString();
		

			switch(item[i].toString().toUpperCase()) { //대문자로 통일
			case "TV" : tvcnt++; break;
			case "COMPUTER" : comcnt++; break;
			case "AUDIO" : aucnt++; break;
		...
		if(sum!=0) {
			...
			//출력 방법 2 : itemlist 변수 없이 바로 Arrays. but, 배열이 비어 있는 부분 null도 함께 출력됨
			//System.out.println("구매하신 제품은 " + Arrays.toString(item) + "이고");
			...
			System.out.println(itemcnt + "이므로 총" + i + "대 입니다.");
