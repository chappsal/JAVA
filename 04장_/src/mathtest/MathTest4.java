...
public class MathTest4 {
	...
	public static void main(String[] args) {
		...
		mathTestEnd: while (true) {
			 ...
			 * System.out.println("다시 입력하세요."); continue; // 아래 내용을 실행하지 않고 위로 올라감 }
			 * 
			 */
			/*
			 * try{예외가 발생할 가능성이 있는 부분} catch(){예외 발생하면 처리}
			 * 
			 */

			// 3. String -> 정수 변환 ("1" "2" "3" "4" / "AS"

			int menu = Integer.parseInt(tmp); // 주의 : "10진수로 된 문자열" -> int로 변환 가능
			try {
				menu = Integer.parseInt(tmp); // "1"->1, "AS"는 예외발생
			} catch (Exception e) {
